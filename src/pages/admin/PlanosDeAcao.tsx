            ...
            return (
              ...
              >
                ...
                <div className={cn('h-9 w-9 rounded-lg flex items-center justify-center shrink-0',
                  atrasado ? 'bg-red-100' : p.status === 'concluido' ? 'bg-emerald-100' : p.status === 'em_andamento' ? 'bg-amber-100' : 'bg-blue-100'
                )}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <p className="text-sm font-semibold text-foreground truncate">{p.descricao_acao}</p>
                  </div>
                    ...
                    {p.prazo && (
                      <>
                        ...
                        <span className={cn('inline-flex items-center gap-1 text-[11px]',
                          atrasado ? 'text-red-600 font-semibold' : 'text-muted-foreground'
                        )}>
                          <Calendar className="h-3 w-3" />
                          {format(new Date(p.prazo + 'T00:00:00'), 'dd/MM/yy')}
                          {atrasado && dias !== null && (
                            <span className="ml-1 rounded-full bg-red-100 text-red-700 px-1.5 py-0.5 text-[9px] font-bold">
                              {Math.abs(dias)}d atrás
                            </span>
                          )}
                          {!atrasado && dias !== null && dias <= 3 && dias >= 0 && (
                            <span className="ml-1 rounded-full bg-amber-100 text-amber-700 px-1.5 py-0.5 text-[9px] font-bold">
                              {dias}d
                            </span>
                          )}
                        </span>
