                ...
                {dayLogs.map(log => {
                  ...
                  return (
                    <div key={log.id} className="flex items-center gap-4 px-5 py-3 transition-colors group hover:bg-muted/30">
                      {/* Action icon */}
                      <div className={cn('h-9 w-9 rounded-lg flex items-center justify-center shrink-0', act.color.split(' ')[1])}>
                        <ActIcon className={cn('h-4 w-4', act.color.split(' ')[0])} />
                      </div>

                      {/* Content */}
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                          <span className={cn('inline-flex items-center gap-1 rounded-md px-1.5 py-0.5 text-[10px] font-medium', tbl.color)}>
                            <TblIcon className="h-3 w-3" />{tbl.label}
                          </span>
                          <span className={cn('inline-flex items-center rounded-md px-1.5 py-0.5 text-[10px] font-bold', act.color)}>
                            {act.label}
                          </span>
                        ...
                        {changes.length > 0 && (
                          <p className="text-[11px] text-muted-foreground truncate">
                            Campos: {changes.map(c => c.field).join(', ')}
                          </p>
                        )}
                      </div>
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        <Button variant="ghost" size="icon" className="h-7 w-7 opacity-0 group-hover:opacity-100 transition-opacity" onClick={() => setDetailLog(log)}>
