        ...
        <div className="rounded-xl border bg-card shadow-sm overflow-hidden divide-y divide-border/50">
          {filteredMetas.map(g => {
            ...
            return (
              ...
              >
                {/* Scope icon */}
                <div className={cn('h-10 w-10 rounded-lg flex items-center justify-center shrink-0', scopeConf.bg)}>
                  <ScopeIcon className={cn('h-5 w-5', scopeConf.color)} />
                </div>

                {/* Main content */}
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="inline-flex items-center rounded-md bg-primary/10 text-primary px-2 py-0.5 text-xs font-mono font-bold">
                      {g.indicators?.nome ?? '—'}
                    </span>
                    <span className="text-sm font-bold text-foreground">
                      {g.valor_meta} {g.indicators?.unidade_medida ?? ''}
                    </span>
                  </div>
                  <div className="flex items-center gap-2 flex-wrap">
                    {/* Scope badge */}
                    <span className={cn('inline-flex items-center gap-1 rounded-md px-1.5 py-0.5 text-[10px] font-medium', scopeConf.bg, scopeConf.color)}>
                      <ScopeIcon className="h-3 w-3" />{scopeConf.label}
                    </span>

                    {/* User name for individual */}
                    {g.user_id && g.users?.nome && (
                      <>
                        <span className="text-muted-foreground/40">•</span>
                        <div className="flex items-center gap-1">
                          <Avatar className="h-4 w-4">
                            <AvatarFallback className="bg-primary/10 text-primary text-[7px] font-semibold">
                              {getInitials(g.users.nome)}
                            </AvatarFallback>
                          </Avatar>
                          <span className="text-xs text-muted-foreground">{g.users.nome}</span>
                        </div>
                      </>
                    )}
                    ...
                    {g.units?.nome && (
                      <>
                        ...
                        <span className="inline-flex items-center gap-1 text-[11px] text-muted-foreground">
                          <Building2 className="h-3 w-3" />{g.units.nome}
                        </span>
                      </>
                    )}
                    ...
                    <span className="inline-flex items-center gap-1 text-[11px] text-muted-foreground hidden sm:inline-flex">
                      ...
                      {g.vigencia_fim ? format(new Date(g.vigencia_fim + 'T00:00:00'), 'dd/MM/yy') : '∞'}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => { setToggleTarget(g); setConfirmOpen(true); }}>
