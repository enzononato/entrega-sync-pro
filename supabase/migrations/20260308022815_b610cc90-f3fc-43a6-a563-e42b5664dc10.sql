...
-- Drop and recreate each policy as PERMISSIVE (default)
...
CREATE POLICY "Users can read applicable shared goals" ON goals FOR SELECT TO authenticated USING (
  (user_id IS NULL) AND (
    (worker_type IN (SELECT users.worker_type FROM users WHERE users.auth_user_id = auth.uid()))
    OR (unidade_id IN (SELECT users.unidade_id FROM users WHERE users.auth_user_id = auth.uid()))
