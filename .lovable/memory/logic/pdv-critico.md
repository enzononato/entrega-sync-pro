---
name: PDV Crítico
description: Indicador mensal exclusivo para motoristas, conta feedbacks "Relevante" da planilha BEES
type: feature
---
...
- Meta: **5** (`valor_meta`). Desafio: **15** (`valor_desafio`).
- Lógica: **menor é melhor**. `status='dentro_meta'` se valor <= 5; `status_desafio='atingiu'` se valor <= 15.
...
- Semanal: `pdv_critico_feedbacks.semana` permite breakdown por semana no front (group by user + semana). Não é gravado em `user_indicator_daily`.
...
- Detecção de duplicidade: `(cpf, mes_num, ano, semana, codigo_cliente, comentario[:60])`. Constraint única no banco no mesmo conjunto.
